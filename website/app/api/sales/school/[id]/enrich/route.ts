...
import { genereerTypeSchoolVoorstel } from "@/lib/sales/enrichment";

// Sales-assistent V1 (2026-08-14) — handmatig getriggerde verrijking
// ("Onderwijstype herkennen") vanaf het schooldetail. V1: uitsluitend
// Type school (zie lib/sales/enrichment.ts).
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const resultaat = await genereerTypeSchoolVoorstel(payload, schoolId);
    return NextResponse.json(resultaat);
  } catch (error) {
    const boodschap = error instanceof Error ? error.message : String(error);
    console.error("[api/sales/school/enrich] mislukt:", boodschap);
