...
- **Private store**: de Blob store is bewust `private` — bestanden zijn dus niet via een kale publieke URL op te vragen. `lib/knowledge/manuals-blob.ts`'s `readManualBlob()` leest ze terug via `@vercel/blob`'s geauthenticeerde `get()` (token of, in productie, Vercel OIDC), niet via `fetch(url)`.
