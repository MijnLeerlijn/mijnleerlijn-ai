...
const LESPLAN_ANALYTICS_ITEM: NavItem = {
  id: "lesplan-generator", label: "Lesplan Generator", href: "/admin/collections/lesplan-analytics",
  icon: BarChart3, color: "purple", description: "Bekijk welke leerdoelen, vakgebieden en groepen in de Lesplan Generator worden gevraagd.",
  permission: { type: "collection", slug: "lesplan-analytics" },
};

const GEBRUIKERS_ITEM: NavItem = {
  id: "gebruikers", permissionId: "algemeen.gebruikers", label: "Gebruikers & rechten", href: "/admin/collections/users",
  icon: Users, color: "green", description: "Beheer accounts en bepaal welke onderdelen iemand mag zien.",
...
export function BeheerNavLinks() {
  ...
  const eersteHref = groups.flatMap((group) => group.items).at(0)?.href ?? "/admin";
    ...
    <>
      ...
      {groups.map((group) => (
        <NavGroup key={group.id} label={group.label}>
          ...
          {!restricted && group.mutedItems.length > 0 && (
            <>
              <div className="ml-nav-muted-label">Technisch</div>
              {group.mutedItems.map((item) => <div className="ml-nav-link--muted" key={item.href}><NavLink item={item} pathname={pathname} gekleurd={false} /></div>)}
            </>
          )}
        </NavGroup>
      ))}

      {!restricted && magGebruikersAanmaken && !beheerZichtbaar && (
        <NavGroup label="Beheer"><NavLink item={GEBRUIKERS_ITEM} pathname={pathname} /></NavGroup>
      )}
    </>
  );
}