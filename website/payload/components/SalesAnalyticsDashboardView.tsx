...
export function SalesAnalyticsDashboardView() {
  ...
  return <div className={s.root}>
    <header className={s.header}><div><h1>Sales Dashboard</h1><p>Commerciële groei, klanten, licenties en omzet uit Monday.</p></div><button className={s.button} type="button" onClick={sync} disabled={syncBezig}>{syncBezig ? "Synchroniseren…" : "Sync met Monday"}</button></header>
    ...
    <div className={s.grid}>
      <Balken titel="Funnel nu" data={data.funnel} /><Balken titel="Klanten per onderwijstype" data={data.klantenPerOnderwijstype} /><Balken titel="Klant geworden" data={data.klantenGeworden} /><Balken titel="Omzet per product" data={data.omzetPerProduct} formatter={euro.format} /><Balken titel="Omzet per maand" data={laatsteMaanden} formatter={euro.format} />
      <section className={s.panel}><h2>Datakwaliteit historie</h2><p><strong>{getal.format(data.historie.volledigeTransities)}</strong> transities bevatten oude én nieuwe waarde.</p><p><strong>{getal.format(data.historie.eersteWaardeZonderVorige)}</strong> registraties bevatten alleen de nieuwe waarde.</p><p className={s.muted}>Monday blijft de operationele bron; historische events worden lokaal vastgelegd zodat latere wijzigingen het verleden niet herschrijven.</p></section>
    </div>
    <footer className={s.footer}><Link href="/admin/sales/scholen">Bekijk scholen</Link><Link href="/admin/sales/acties">Bekijk acties</Link></footer>
  </div>;
}
