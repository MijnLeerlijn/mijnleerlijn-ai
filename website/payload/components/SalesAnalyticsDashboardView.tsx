...
function Balken({ titel, data }: { titel: string; data: { label: string; waarde: number }[] }) {
...
export function SalesAnalyticsDashboardView() {
  ...
  return <div className={s.root}>
    <header className={s.header}>
      <div>
        <h1>{trainingView ? "Trainingen & upsell" : "Sales Dashboard"}</h1>
        <p>{trainingView ? "Trainingen, planning en aanvullende verkoop bij bestaande klanten." : "Scholen, licenties, groei, pipeline en commerciële doelstellingen."}</p>
      </div>
      {magBeheren && <button className={s.button} type="button" onClick={sync} disabled={syncBezig}>{syncBezig ? "Synchroniseren…" : "Sync met Monday"}</button>}
    </header>

    <nav className={s.footer} style={{ marginTop: 0, marginBottom: 22 }}>
      <Link href="/admin/sales">Scholen & licenties</Link>
      <Link href="/admin/sales?view=trainingen">Trainingen & upsell</Link>
    </nav>
    ...
    {trainingView ? <>
      ...
      </> : <section className={s.panel}><h2>Trainingen & upsell</h2><p className={s.muted}>De trainingsdata is op dit moment niet beschikbaar.</p></section>}
    </> : <>
      <div className={s.kpis}>
