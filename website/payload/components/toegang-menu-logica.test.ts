...
describe("alleItemIds", () => {
  it("bevat exact de 8 Trainers-permissie-ID's uit de opdracht", () => {
...
describe("berekenGroepTelling", () => {
  it("'alles' is waar wanneer elk item van de groep geselecteerd is", () => {
    ...
    expect(telling).toEqual({ aantalGeselecteerd: 8, totaal: 8, alles: true, niets: false });
  });

  it("'niets' is waar bij een lege selectie", () => {
    const telling = berekenGroepTelling(trainers, new Set());
    expect(telling).toEqual({ aantalGeselecteerd: 0, totaal: 8, alles: false, niets: true });
  });
